  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-5">
          ...
          {TYPES.map(({ id, label, Icon }) => {
            ...
            return (
              <button
                ...
                aria-pressed={on}
                onClick={() => setType(id)}
                className={`flex flex-col items-center gap-1 px-1 py-2 rounded-[9px] border text-xs transition-colors ${on ? 'border-gold text-gold bg-gold-soft' : 'border-line-strong text-muted hover:border-brass'}`}
              >
                <Icon size={16} />
                {label}
              </button>
      ...
      <div className="flex flex-col gap-[9px]">
        ...
        <div className="relative">
          <textarea
            ...
            rows={5}
            className="w-full bg-surface-2 border border-line rounded-lg px-3 py-2 text-ink focus:outline-none focus:border-brass"
          />
      ...
      <button
        ...
        className="self-start bg-brass hover:brightness-110 disabled:opacity-60 text-accent-ink font-semibold px-8 py-3 rounded-lg transition-colors"
      >

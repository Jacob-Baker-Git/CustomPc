...
const LEGEND = [
  ['bg-good', '60+ smooth'],
  ['bg-ok', '30–59 playable'],
  ['bg-bad', 'under 30 struggles'],
]
...
export default function GamePerformancePanel() {
  ...
  return (
    <div className={`${PANEL} p-3`}>
      ...
      {!cpu || !gpu ? (
        <p className="text-xs text-muted py-2">Select a CPU + GPU to see game FPS.</p>
      ) : (
        <>
          <p className="text-xs text-muted mb-2">
            Based on <span className="text-ink">{cpu.name}</span> + <span className="text-ink">{gpu.name}</span>
          </p>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[11px] text-muted">Graphics</span>
              ...
              {QUALITY_LEVELS.map((q) => (
                <button
                  ...
                  className={`px-2 py-0.5 text-[11px] rounded-lg transition-colors
                    ${quality === q ? 'bg-accent text-ink' : 'text-muted hover:text-ink'}`}
                >
                  {QUALITY_LABEL[q]}
                </button>
              ))}
